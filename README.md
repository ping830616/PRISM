...
After both pre-freeze platform datasets are copied to the Mac, sections 14-19
audit the 160 admitted runs, create fingerprinted nonoverlapping semantic
blocks, compare the DICE ridge baseline with VAR/persistence/EWMA/CUSUM and
sequential conformal evidence, measure destination-platform calibration, and
write the guarded method-freeze artifact. Section 20 contains only a read-only,
P1 NVIDIA L40S feasibility gate; it cannot start GPU collection. Section 21
runs analysis self-checks. A failed development gate keeps the locked test
closed rather than selecting a zero-alert detector.
